      ...
      <Card className="max-w-4xl mx-auto rounded-3xl shadow-xl border-0 relative bg-rose-50 mt-4">
            ...
            <Dialog>
                ...
                <DialogContent className="sm:max-w-sm">
                    ...
                    <div className="flex items-center justify-center p-4 bg-white rounded-lg mt-4">
                        {user && <QRCode value={user.id} size={256} includeMargin={true} />}
                    </div>
                </DialogContent>
            </Dialog>
        </div>
        ...
        <CardContent className="pt-28 pb-10 px-10">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            {user.role === 'student' && (
              <div className="md:col-span-2 border-t mt-6 pt-6">
                <h2 className="text-lg font-bold mb-4">Emergency Contact Information</h2>
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <Label>Contact Person Name</Label>
                    <Input value={formData.emergencyContactName} onChange={e => setFormData({ ...formData, emergencyContactName: e.target.value })} className="rounded-full bg-rose-100 border-rose-200" />
                  </div>
                  <div>
                    <Label>Phone Number</Label>
                    <Input value={formData.emergencyContactPhone} onChange={e => setFormData({ ...formData, emergencyContactPhone: e.target.value })} className="rounded-full bg-rose-100 border-rose-200" />
                  </div>
                  <div className="md:col-span-2">
                    <Label>Address</Label>
                    <Input value={formData.emergencyContactAddress} onChange={e => setFormData({ ...formData, emergencyContactAddress: e.target.value })} className="rounded-full bg-rose-100 border-rose-200" />
                  </div>
                </div>
              </div>
            )}
